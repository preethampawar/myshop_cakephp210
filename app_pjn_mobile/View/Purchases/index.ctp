...
<p><?php echo $this->Html->link('Import Purchases', array('controller'=>'purchases', 'action'=>'uploadCsv'));?></p><br>
...
<h2>Recent Purchases</h2>
	...
	<table class='table' style="width:100%;">
		<thead>
			<tr>
				<th>S.No</th>
				<th><?php echo $this->Paginator->sort('purchase_date', 'Date'); ?></th>
				...
				<th>Unit Price</th>
				<th>Total Amount</th>
				<th><?php echo $this->Paginator->sort('invoice_name', 'Invoices'); ?></th>
				<th>Actions</th>
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				...
				<td><?php echo ($row['Purchase']['invoice_id']) ? $this->Html->link($row['Purchase']['invoice_name'], array('controller'=>'invoices', 'action'=>'details', $row['Purchase']['invoice_id']), array('title'=>'Invoice Details')) : '-';?></td>
				<td>
					<form method="post" style="" name="purchase_product_<?php echo $row['Purchase']['id'];?>" id="purchase_product_<?php echo $row['Purchase']['id'];?>" action="<?php echo $this->Html->url("/purchases/removeProduct/".$row['Purchase']['id']);?>">						
						<a href="#" name="Remove" onclick="if (confirm('Are you sure you want to delete this product - <?php echo $row['Purchase']['product_name'];?> from the list?')) { $('#purchase_product_<?php echo $row['Purchase']['id'];?>').submit(); } event.returnValue = false; return false;" class="btn btn-danger btn-xs">
