...
class StoresController extends AppController
{
    ...
    public function getStoreInfo($storeID = null)
    {
        ...
        $data = $this->Store->find('first', array('conditions' => $conditions, 'recursive' => 2));
    ...
    public function index()
    {
        ...
        $stores = $this->Store->find('all', array('conditions' => $conditions, 'order' => array('Store.created ASC')));
    ...
    public function edit($storeID = null)
    {
        ...
        if (!($storeInfo = $this->getStoreInfo($storeID))) {
            $this->Session->setFlash('Store not found', 'default', array('class' => 'error'));
            $this->redirect('/stores/');
        }
    ...
    public function delete($storeID = null)
    {
        if ($this->request->isPost()) {
            if (!($storeInfo = $this->getStoreInfo($storeID))) {
                $this->Session->setFlash('Store not found');
            } else {
                ...
                $this->Session->setFlash('Store "' . $storeInfo['Store']['name'] . '" has been removed', 'default', array('class' => 'success'));
            }
        } else {
...
//				$this->Store->query("delete from brand_products where store_id='$storeID'");	// remove records from brand_products table
                ...
                $this->Session->setFlash('Test data has been removed', 'default', array('class' => 'success'));
