...
class QuotationsController extends AppController
{
    ...
    function index()
    {
        $conditions = array('Quotation.store_id' => $this->Session->read('Store.id'));
    ...
    function details($quotationID, $downloadType = 'quotation')
    {
        ...
        $this->Quotation->bindModel(array('hasMany' => array('EntityItem'), 'belongsTo' => array('Store', 'User')));
        $conditions = array('Quotation.store_id' => $companyID, 'Quotation.id' => $quotationID);
        if ($this->Session->read('UserCompany.user_level') == '2') {
            $conditions[] = array('Quotation.user_id' => $this->Auth->user('id'));
        }
