...
class InvoicesController extends AppController
{
    ...
    public function getInvoiceInfo($invoiceID = null, $type = null)
    {
        ...
        $conditions = array('Invoice.id' => $invoiceID, 'Invoice.store_id' => $this->Session->read('Store.id'));
        if ($type) {
            $conditions['Invoice.invoice_type'] = ($type == 'purchase') ? 'purchase' : 'sale';
        }
        return $this->Invoice->find('first', array('conditions' => $conditions, 'recursive'=>'-1'));
    }

    public function index($type = null)
    {
        $this->Session->delete('Invoice');
        $conditions = array('Invoice.store_id' => $this->Session->read('Store.id'));
        if ($type) {
            $conditions['Invoice.invoice_type'] = ($type == 'sale') ? 'sale' : 'purchase';
        }
        $invoices = $this->Invoice->find('all', array('conditions' => $conditions, 'order' => array('Invoice.invoice_date DESC', 'Invoice.created DESC')));
                            ...
                            if ($this->Invoice->find('first', array('conditions' => $conditions))) {
                                ...
                                if ($this->Invoice->save($data)) {
                                    if($invoiceInfo['Invoice']['invoice_type'] == 'sale') {
                                        ...
                                        App::uses('Sale', 'Model');
                                        $this->Sale = new Sale;
                                        $fields = array('Sale.sale_date' => "'" . $invoiceDate . "'", 'Sale.invoice_name' => "'".$data['Invoice']['name']."'");
                                        $conditions = array('Sale.invoice_id' => $invoiceID);
                                        $this->Sale->recursive = '-1';
                                        $this->Sale->updateAll($fields, $conditions);
                                        // update franchise details
                                        if ($hasFranchise) {
                                            $fields = array('Sale.franchise_id' => $franchiseId, 'Sale.franchise_name' => $franchiseName);
                                            $conditions = array('Sale.invoice_id' => $invoiceID);
                                            $this->Sale->recursive = '-1';
                                            $this->Sale->updateAll($fields, $conditions);
                                        }
