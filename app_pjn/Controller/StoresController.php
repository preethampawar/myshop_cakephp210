...
class StoresController extends AppController
{
	...
	public function edit($storeID = null)
	{
		...
		if (!($storeInfo = $this->getStoreInfo($storeID))) {
			$this->Session->setFlash('Store not found', 'default', ['class' => 'error']);
			$this->redirect('/stores/');
		}
	...
	public function delete($storeID = null)
	{
		if ($this->request->isPost()) {
			if (!($storeInfo = $this->getStoreInfo($storeID))) {
				...
				$this->Session->setFlash('Store "' . $storeInfo['Store']['name'] . '" has been removed', 'default', ['class' => 'success']);
			}
		} else {
...
//				$this->Store->query("delete from brand_products where store_id='$storeID'");	// remove records from brand_products table
				...
				$this->Session->setFlash('Test data has been removed', 'default', ['class' => 'success']);
	...
	private function updateFields($storeID, $field, $value)
	{
		...
		if ($this->Store->save($data)) {
			...
			$this->Session->setFlash($msg, 'default', ['class' => 'error']);
		}
		$this->redirect('/stores/');
	}
	...
	public function settings($storeId)
	{
		...
		if ($this->request->is('post') or $this->request->is('put')) {
			...
			if ($data) {
				...
				$this->Session->setFlash('Settings updated successfully', 'default', ['class' => 'success']);
