...
class SalesController extends AppController
{
        ...
        if ($this->request->isPost() or $this->request->isPut()) {
            ...
            if (!$error) {
                ...
                if ($this->Sale->save($data)) {
                    ...
                    $this->redirect(array('controller' => 'sales', 'action' => 'add'));
                }
            }
        } else {
    ...
    function addProduct()
    {
        ...
        if ($this->request->isPost() or $this->request->isPut()) {
            ...
            if (!$error) {
                ...
                if ($this->Sale->save($data)) {					
                    ...
                    $this->Session->setFlash($msg, 'default', array('class' => 'success'));
                    $this->redirect(array('controller' => 'sales', 'action' => 'addProduct'));
                }
            }
        } else {
            if ($this->Session->check('selectedProductID')) {
        ...
        $conditions = array('Sale.invoice_id' => $invoiceInfo['Invoice']['id']);
        $invoiceProducts = $this->Sale->find('all', array('conditions' => $conditions, 'order' => 'Sale.created ASC', 'recursive' => '2'));
    ...
    public function removeProduct($saleId = null)
    {
        if ($this->request->isPost()) {
            if ($saleInfo = $this->CommonFunctions->getSaleInfo($saleId)) {
                ...
                $this->Session->setFlash('"' . $saleInfo['Sale']['product_name'] . '" removed from the list', 'default', array('class' => 'success'));
            } else {
    ...
    public function addClosingStock()
    {
        ...
        if ($this->request->isPost() or $this->request->isPut()) {
            ...
            if (!$error) {
                ...
                if ($this->Sale->save($data)) {
                    ...
                    $msg = $productInfo['Product']['name'] . ' successfully added to Sales list';
                    $this->Session->setFlash($msg, 'default', array('class' => 'success'));
                    $this->redirect(array('controller' => 'sales', 'action' => 'addClosingStock'));
                }
            }
        } else {
            if ($this->Session->check('selectedProductID')) {
        ...
        $conditions = array('Sale.store_id' => $this->Session->read('Store.id'), 'Sale.reference' => '#ClosingStock');
        $saleProducts = $this->Sale->find('all', array('conditions' => $conditions, 'order' => 'Sale.created DESC', 'recursive' => '0', 'limit' => '10'));
    ...
    public function addAllClosingStock()
    {
        ...
        $conditions = array('Product.store_id' => $this->Session->read('Store.id'));
        $productsList = $this->Product->find('list', array('conditions' => $conditions, 'order' => 'Product.name', 'recursive' => '-1'));
        $productsSellingPrice = $this->Product->find('list', array('conditions' => $conditions, 'fields' => array('Product.id', 'Product.unit_selling_price'), 'order' => 'Product.name', 'recursive' => '-1'));
        $productsInfo_tmp = $this->Product->find('all', array('conditions' => $conditions, 'order' => 'Product.name', 'recursive' => '1'));
        $productsInfo = null;
        if ($productsInfo_tmp) {
