...
<h2 class="mb-3"><i class="fa fa-list-alt"></i> Closing Stock Report</h2>
		...
		<tbody>
			<tr>
				<td>
					...
					<input type="date" id="fromDate" name="fromDate" class="form-contorl form-contorl-sm" value="<?php echo $fromDate;?>" required>
				</td>
				<td>&nbsp;&nbsp;</td>				
				<td>
					<label for="toDate">To Date:</label><br>
					<input type="date" name="toDate" class="form-contorl form-contorl-sm" value="<?php echo $toDate;?>" required>
				</td>
			</tr>
		</tbody>
	</table>
	...
	?>
	<hr>
	<p class="small text-center">Result from "<?php echo Date('d-m-Y', strtotime($fromDate));?>" to "<?php echo Date('d-m-Y', strtotime($toDate));?>"</p>
	<?php
	if($sales) {
		$totalAmount = 0;
