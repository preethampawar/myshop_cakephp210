...
if ($productsInfo) {
	...
	<div id="AddBreakageProductDiv" class="well">
		...
		<div id="paramsDiv">
			...
			<div style="float:left; clear:none;">
				<?php
				echo $this->Form->input('breakage_stock_qty', ['type' => 'number', 'value' => '0', 'min' => '0', 'max' => '99999', 'label' => 'Breakage Quantity', 'required' => true, 'oninput' => 'setTotalPrice()', 'title' => 'Values should be between 0 to 99999', 'style' => 'width:80px;']);
				?>
			...
			<div style="float:left; clear:none;">
				<?php echo $this->Form->input('breakage_date', ['label' => 'Breakage Date', 'required' => true, 'type' => 'date']); ?>
			</div>
			<div style="float:left; clear:none; margin-top:25px;">
				<?php echo $this->Form->submit('Add Product', ['div' => false, 'id' => 'AddBreakageProduct', 'title' => '', 'type' => 'submit', 'onclick' => 'return submitButtonMsg()']); ?>
			</div>
		...
		<table class='table'>
			...
			foreach ($breakageProducts as $row) {
				...
				<tr>
					...
					<td>
						<form method="post" style="" name="invoice_remove_product_<?php echo $row['Breakage']['id']; ?>"
							  id="invoice_remove_product_<?php echo $row['Breakage']['id']; ?>"
