...
class StoresController extends AppController {
	...
	public function getStoreInfo($storeID=null) {		
		...
		$conditions = null;
		if($this->Session->read('manager') != '1') {
			$conditions = array('Store.id'=>$storeID, 'Store.user_id'=>$this->Auth->user('id'));
		} else {
			$conditions = array('Store.id'=>$storeID);
		}
		return $this->Store->find('first', array('conditions'=>$conditions));
	}
	...
	public function add() {
		...
		if($this->request->data) {
			...
			if(isset($data['Store']['name'])) {
				if($data['Store']['name']=trim($data['Store']['name'])) {
					$conditions = array('Store.name'=>$data['Store']['name'], 'Store.user_id'=>$this->Auth->user('id'));
					if($this->Store->find('first', array('conditions'=>$conditions))) {
		...
		($errorMsg) ? $this->Session->setFlash($errorMsg, 'default', array('class'=>'alert alert-danger')) : null;
    ...
    public function index()
    {
        ...
        if ($this->Session->read('manager') != '1') {
            $conditions = array('Store.user_id' => $this->Auth->user('id'));
        }

        $storesTmp = $this->Store->find('all', array('conditions' => $conditions, 'order' => array('Store.expiry_date desc')));
        $stores = $storesTmp;
	...
	public function edit($storeID=null) {
		...
		if(!($storeInfo = $this->getStoreInfo($storeID))) {
			$this->Session->setFlash('Store not found', 'default', array('class'=>'alert alert-danger'));
			$this->redirect('/stores/');
		}
		...
		if($this->request->data) {
			...
			if(isset($data['Store']['name'])) {
				if($data['Store']['name']=trim($data['Store']['name'])) {	
					...
					$conditions = array('Store.name'=>$data['Store']['name'], 'Store.user_id'=>$storeInfo['Store']['user_id'], 'Store.id <>'=>$storeID);
					if($this->Store->find('first', array('conditions'=>$conditions))) {
						$errorMsg = "'".$data['Store']['name']."'". ' already exists';
					}
					else {
	...
	public function delete($storeID=null) {
		if($this->request->isPost()) {
			if(!($storeInfo = $this->getStoreInfo($storeID))) {
				...
				$this->Session->setFlash('Store "'.$storeInfo['Store']['name'].'" has been removed', 'default', array('class'=>'alert alert-success'));
			}
		}
		else {
			$this->Session->setFlash('Unauthorized access', 'default', array('class'=>'alert alert-danger'));
		}
...
//				$this->Store->query("delete from brand_products where store_id='$storeID'");	// remove records from brand_products table
				...
				$this->Session->setFlash('Test data has been removed', 'default', array('class'=>'alert alert-success'));
	...
	public function createbackup() {
		...
		$this->redirect(array('action'=>'dbbackuplist'));
		exit;
	}
