...
<nav aria-label="breadcrumb">
	<ol class="breadcrumb">
		...
		<li class="breadcrumb-item"><a href="/admin/categories/showProducts/<?php echo $categoryID; ?>"><?php echo $categoryInfo['Category']['name']; ?></a></li>
...
<div id="EditProduct<?= $productInfo['Product']['id'] ?>">
	<div class="mt-3 d-flex justify-content-between align-items-center">
		...
		<a href="/admin/categories/showProducts/<?php echo $categoryID; ?>" class="btn btn-outline-warning btn-sm ml-3">Cancel</a>
	</div>

	<form action="/admin/products/edit/<?= $productInfo['Product']['id'] ?>/<?= $categoryID ?>" id="ProductAdminEditForm" method="post" accept-charset="utf-8" ref="form">
	<?php
	echo $this->Form->create('Product', ['onsubmit']);
	?>
	...
	<div class="py-3">
		<div class="mt-3 d-flex justify-content-between align-items-center">
			<h6>Images</h6>
			<a href="<?php echo '/admin/images/manageProductImages/' . $productInfo['Product']['id'] . '/' . $categoryID;?>" class="btn btn-info btn-sm">Manage Images</a>
		</div>
		<hr>
		<?php
		if (!empty($productInfo['Product']['images'])) {
			...
			<div class="">
				...
				<div class="d-block">
					...
					foreach ($productImages as $row) {
						...
						?>
						<div class="mr-2 mb-2 shadow-sm rounded float-left clear <?php echo $imageHighlight ? 'border border-warning' : '';?>">
			...
			<div class="mb-2">
				No images found.
				<a href="<?php echo '/admin/images/manageProductImages/' . $productInfo['Product']['id'] . '/' . $categoryID;?>">Click here</a> to add images.
			</div>
			<?php
		}
		?>
	...
	<div class="mt-2">
		<div class="pt-3 d-flex justify-content-between align-items-center">
			...
			<a href="<?php echo '/admin/images/manageProductImages/' . $productInfo['Product']['id'] . '/' . $categoryID;?>" class="btn btn-info btn-sm">+ Add Category</a>
		</div>
		<hr>
		...
		<div>
			...
			<div class="my-3">
				<label for="ProductDiscount" class="form-label">Discount Value (<?= $this->App->price('') ?>)</label>
				<input
					v-model="discount"
					type="number"
					id="ProductDiscount"
...
<br>
		<div class="my-3 py-3 d-inline">
			<button v-if="submitDisabled" type="button" class="btn btn-secondary btn-sm disabled">Save Changes</button>
			<button v-else type="submit" class="btn btn-primary btn-sm">Save Changes</button>
			<a href="/admin/categories/showProducts/<?php echo $categoryID; ?>" class="btn btn-outline-secondary btn-sm ml-3">Cancel</a>
		</div>
	</div>
