...
<div id="content">
	<section>
		...
		echo $this->Form->input('Category.active', ['label' => 'Active', 'title' => 'Status Active', 'style' => 'width:10px; margin-right:5px;', 'before' => 'Status: ', 'after' => '<br><br>']);
		echo $this->Form->input('Category.name', ['label' => false, 'title' => 'Add new category', 'style' => 'width:300px; float:left; margin-right:20px;']);
		echo $this->Form->submit('Update', ['class' => 'floatLeft']);
		echo $this->Form->end();
		?>
		<div class='clear'>&nbsp;</div>
