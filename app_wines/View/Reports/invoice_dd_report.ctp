	...
	if (isset($invoices) and !empty($invoices)) {
		...
		<table class='table' style="width:100%;">
			...
			<tr>
				...
				<th style="width:200px;">New Retailer Professional Tax</th>
			...
			foreach ($invoices as $row) {
				...
				<tr>
					...
					<!--                    <td>-->
					<?php //echo $row['Invoice']['retail_shop_excise_turnover_tax']; ?><!--</td>-->
					<td><?php echo $row['Invoice']['special_excise_cess']; ?></td>
					<td><?php echo $row['Invoice']['tcs_value']; ?></td>


					<td><?php echo $row['Invoice']['new_retailer_prof_tax']; ?></td>
			...
			<tr>
				...
				<!--                <td title="Total Retail Shop Excise Turnover Tax">-->
				<?php //echo $total_retail_shop_excise_turnover_tax; ?><!--</td>-->
				<td title="Special Excise Cess"><?php echo number_format($total_special_excise_cess, '2', '.', ''); ?></td>
				<td title="Total TCS Value"><?php echo number_format($total_tcs_value, '2', '.', ''); ?></td>


				<td title="Total New Retailer Professional Tax"><?php echo number_format($total_new_retailer_prof_tax, '2', '.', ''); ?></td>
		...
		<table class="table table-values" style="width:400px;">
			...
			<!--                <td>-->
			<?php //echo number_format($total_retail_shop_excise_turnover_tax, '2', '.', ''); ?><!--</td>-->
			<!--            </tr>-->
			<tr>
...
//					'Total Retail Shop Excise Turnover Tax',
		...
		foreach ($invoices as $row) {
			...
			$tmp[] = $row['Invoice']['mrp_rounding_off'];
			$tmp[] = $row['Invoice']['invoice_value'] + $row['Invoice']['special_margin'] + $row['Invoice']['mrp_rounding_off'];
...
//			$tmp[] = $row['Invoice']['retail_shop_excise_turnover_tax'];
			...
			$tmp[] = $row['Invoice']['new_retailer_prof_tax'];
			$tmp[] = $row['Invoice']['credit_balance'];
			$csv .= implode($tmp, ',') . "\r\n";
		}
		$csv .= " , , Total: ,$total_invoice_value, $total_mrp_rounding_off, $total_net_invoice_value, $total_dd_amount, , $total_special_excise_cess, $total_tcs_value, $total_new_retailer_prof_tax, ";
		...
		$csv .= 'Total MRP Rounding Up,' . $total_mrp_rounding_off . "\r\n";
...
//		$csv .= 'Total Retail Shop Excise Turnover Tax,' . $total_retail_shop_excise_turnover_tax . "\r\n";
		...
		$csv .= 'New Retailer Professional Tax,' . $total_new_retailer_prof_tax . "\r\n";
